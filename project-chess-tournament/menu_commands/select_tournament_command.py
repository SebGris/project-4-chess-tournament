...
class SelectTournamentCommand(Command):
    ...
    def execute(self):
        if not self.tournament_controller.tournaments:
            ...
            return
        tournament_index = self.tournament_controller.view.get_tournament_selection(self.tournament_controller.tournaments)
        if 0 <= tournament_index < len(self.tournament_controller.tournaments):
            ...
            self.tournament_controller.view.display_tournament_selected(self.tournament_controller.active_tournament.name)
        else:
